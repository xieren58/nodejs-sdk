 ...
 * 
 */
exports.generateWatermarkUrl = function (domain, key, text, fontName, fontSize,
                                         textColor, dissolve, gravity, dx, dy) {
  ...
  return util.format('http://%s/%s?watermark/2/text/%s/font/%s/fontsize/%d/fill/%s/dissolve/%d/gravity/%s/dx/%d/dy/%d',
                     domain, key, encode(text), encode(fontName), fontSize,
                     encode(textColor), dissolve, gravity, dx, dy);
};
